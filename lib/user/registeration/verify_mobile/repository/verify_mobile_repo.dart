...
class VerifyMobileRepo {
  final HttpService httpService;

  VerifyMobileRepo({required this.httpService});

  Future<VerifyMobileModel> verifyEmail({
    ...
    try {
      final response = await httpService.handleVerifyPostRequest(
        "${EndPointURL.baseURL}/api/verifyotp",
        data: {
          "vericode": vericode
        },
       email: email,
      ...
      if (response.statusCode! >= 200 && response.statusCode! <= 300) {
        ...
        return VerifyMobileModel.fromJson(_result);
      } else {
        print(response);
